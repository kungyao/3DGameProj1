...
namespace burningmime.curves
{
    ...
    public abstract class CurveFitBase
    {
        ...
        /// </summary>
        protected readonly List<Vector3> _pts = new List<Vector3>(256);
        ...
        protected bool FitCurve(int first, int last, Vector3 tanL, Vector3 tanR, out CubicBezier curve, out int split)
        {
            ...
            else if(nPts == 2)
            {
                ...
                Vector3 p1 = (tanL * alpha) + p0;
                Vector3 p2 = (tanR * alpha) + p3;
        ...
        /// </summary>
        protected Vector3 GetLeftTangent(int last)
        {
            List<Vector3> pts = _pts;
            List<float> arclen = _arclen;
            float totalLen = arclen[arclen.Count - 1];
            Vector3 p0 = pts[0];
            Vector3 tanL = VectorHelper.Normalize(pts[1] - p0);
            Vector3 total = tanL;
            float weightTotal = 1;
            ...
            for(int i = 2; i <= last; i++)
            {
                ...
                Vector3 v = VectorHelper.Normalize(pts[i] - p0);
                total += v * weight;
                weightTotal += weight;
            }
        ...
        /// </summary>
        protected Vector3 GetCenterTangent(int first, int last, int split)
        {
            List<Vector3> pts = _pts;
            List<float> arclen = _arclen;
            ...
            for(int i = Math.Max(first, split - MID_TANGENT_N_PTS); i < split; i++)
            {
                ...
                float weight = t * t * t;
                Vector3 v = VectorHelper.Normalize(pts[i] - pSplit);
                total += v * weight;
                weightTotal += weight;
            }
            Vector3 tanL = VectorHelper.Length(total) > EPSILON && weightTotal > EPSILON ? 
            ...
            total = default(Vector3);
            weightTotal = 0;
            for(int i = split + 1; i <= rMax; i++)
            {
                float ti = 1 - ((arclen[i] - splitLen) / partLen);
                float weight = ti * ti * ti;
                Vector3 v = VectorHelper.Normalize(pSplit- pts[i]);
                total += v * weight;
                weightTotal += weight;
            }
            Vector3 tanR = VectorHelper.Length(total) > EPSILON && weightTotal > EPSILON ?
                VectorHelper.Normalize(total / weightTotal) :
        ...
        /// </summary>
        protected CubicBezier GenerateBezier(int first, int last, Vector3 tanL, Vector3 tanR)
        {
            List<Vector3> pts = _pts;
            List<float> u = _u;
            int nPts = last - first + 1;
            Vector3 p0 = pts[first], p3 = pts[last]; // first and last points of curve are actual points on data
            ...
            for(int i = 1; i < nPts; i++)
            {
                ...
                Vector3 s = (p0 * t0) + (p0 * t1) + (p3 * t2) + (p3 * t3); // NOTE: this would be Q(t) if p1=p0 and p2=p3
                Vector3 v = pts[first + i] - s;
            ...
            {
                Vector3 p1 = (tanL * alphaL) + p0;
                Vector3 p2 = (tanR * alphaR) + p3;
                return new CubicBezier(p0, p1, p2, p3);
            }
        }
        ...
        protected void Reparameterize(int first, int last, CubicBezier curve)
        {
            ...
            for(int i = 1; i < nPts; i++)
            {
                Vector3 p = pts[first + i];
                float t = u[i];
                float ti = 1 - t;

                // Control vertices for Q'
                Vector3 qp0 = (curve.p1 - curve.p0) * 3;
                Vector3 qp1 = (curve.p2 - curve.p1) * 3;
                Vector3 qp2 = (curve.p3 - curve.p2) * 3;

                // Control vertices for Q''
                Vector3 qpp0 = (qp1 - qp0) * 2;
                Vector3 qpp1 = (qp2 - qp1) * 2;

                // Evaluate Q(t), Q'(t), and Q''(t)
                Vector3 p0 = curve.Sample(t);
                Vector3 p1 = ((ti * ti) * qp0) + ((2 * ti * t) * qp1) + ((t * t) * qp2);
                Vector3 p2 = (ti * qpp0) + (t * qpp1);
        ...
        protected float FindMaxSquaredError(int first, int last, CubicBezier curve, out int split)
        {
            ...
            for(int i = 1; i < nPts; i++)
            {
                Vector3 v0 = pts[first + i];
                Vector3 v1 = curve.Sample(u[i]);
