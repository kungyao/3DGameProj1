...
namespace burningmime.curves
{
    ...
    public sealed class CurveBuilder : CurveFitBase, IEnumerable<CubicBezier>
    {
        ...
        private Vector3 _prev;                                            // most recent point added
        private Vector3 _tanL;                                            // left tangent of current curve (can't change this except on first curve or we'll lose C1 continuity)
        ...
        public AddPointResult AddPoint(Vector3 p)
        {
            Vector3 prev = _prev;
            ...
            if(count != 0)
            {
                ...
                if(td > md)
                {
                    ...
                    Vector3 dir = VectorHelper.Normalize(p - prev);
                    do
                    {
                        Vector3 np = prev + dir * md;
        ...
        private AddPointResult AddInternal(Vector3 np)
        {
            List<Vector3> pts = _pts;
            int last = pts.Count;
            ...
            if(last == 1)
            {
                ...
                Vector3 p0 = pts[0];
                Vector3 tanL = VectorHelper.Normalize(np - p0);
                Vector3 tanR = -tanL;
                _tanL = tanL;
                float alpha = _linDist / 3;
                Vector3 p1 = (tanL * alpha) + p0;
                Vector3 p2 = (tanR * alpha) + np;
            ...
            else
            {
                ...
                Vector3 tanL = lastCurve == 0 ? GetLeftTangent(last) : _tanL;

                // We can always do the end tangent
                Vector3 tanR = GetRightTangent(first);
                ...
                else
                {
                    ...
                    Vector3 tanM1 = GetCenterTangent(first, last, split);
                    Vector3 tanM2 = -tanM1;
        ...
        public void Clear()
        {
            ...
            _totalLength = 0;
            _first = 0;
            _tanL = default(Vector3);
            _prev = default(Vector3);
        }
